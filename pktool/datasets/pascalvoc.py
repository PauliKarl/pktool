...
class PascalVocWriter:
    """Create voc annotation with xml format
    example:
        >imgShape=[height, width, channel]
        >writer = PascalCVocWriter(imgFolderName,imgFileNameWithoutExt,imageShape,imagePath)
        >writer.verified=True or False # default for False 
        >writer.addBndBox([xmin,ymin,xmax,ymax],label,difficult)# difficult(int): 0 or 1
        >writer.save() # default for filename + XML_EXT and targetFile is option
    """
...
class PascalVocReader:
    """ read VOC with xml format
    example:
        >tVocParseReader = PascalVocReader(xmlPath)
        >shapes = tVocParseReader.getShapes()# shapes [label, points, direction, isRotated, difficult]
    """
    ...
    def addShape(self, label, bndbox, difficult):
        ...
        self.shapes.append((label, points, 0, False, difficult))
    ...
    def addRotatedShape(self, label, robndbox, difficult):
        ...
        self.shapes.append((label, points, angle, True, difficult))
